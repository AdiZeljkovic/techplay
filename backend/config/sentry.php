...
/*
|--------------------------------------------------------------------------
| Error tracking — GlitchTip, not Sentry
|--------------------------------------------------------------------------
|
| This is the Sentry Laravel SDK pointed at a self-hosted GlitchTip at
| glitchtip.techplay.gg. GlitchTip speaks the same protocol and takes the same
| DSN format, which is exactly why it was chosen: Sentry's own self-hosted
| build wants 16 GB of RAM and forty containers, and this machine has 7.5 GB
| and a site to serve. GlitchTip does the same job in about 260 MB, and events
| never leave our own server.
|
| Two settings are deliberately off:
|
|   traces_sample_rate    Performance tracing counts every HTTP request as a
|                         transaction. Netdata already measures how fast this
|                         box answers, at one-second resolution and for free.
|                         Paying storage for a second, coarser copy of that
|                         buys nothing.
|
|   profiles_sample_rate  Profiling needs the excimer extension, which is not
|                         installed, and GlitchTip does not read profiles.
|
| What we want from this file is the thing nothing else can do: an exception,
| with its stack, the moment it happens.
|
*/
