...
export default function TrackGameButton({ slug, gameName, variant = "full", wrapperClassName }: Props) {
    ...
    if (!user) {
        ...
        return (
            <a href="/login" className="w-full flex items-center justify-center gap-2 py-3 rounded-xl bg-[var(--accent)] hover:bg-[var(--accent-hover)] text-white font-bold text-[13px] uppercase tracking-wider transition-colors shadow-[0_0_20px_rgba(252,65,0,0.2)]">
    ...
    return (
        <div className={`relative ${wrapperClassName ?? "w-full"}`}>
            {currentMeta ? (
                ...
                <button onClick={() => setOpen((v) => !v)} disabled={busy}
                    className="w-full flex items-center justify-center gap-2 py-3 rounded-xl bg-[var(--accent)] hover:bg-[var(--accent-hover)] text-white font-bold text-[13px] uppercase tracking-wider transition-colors shadow-[0_0_20px_rgba(252,65,0,0.2)]">
