...
export default function GameTrailersPlayer({ movies }: Props) {
    ...
    return (
        <div className="bg-[#0f1221]/80 border border-white/5 rounded-[var(--radius-panel)] p-6 shadow-2xl">
            ...
            <div className="rounded-[var(--radius-panel)] overflow-hidden border border-white/10 mb-4 aspect-video bg-black">
                {embedUrl ? (
                    <iframe
                        key={embedUrl}
                        src={embedUrl}
                ...
                <div className="flex gap-3 overflow-x-auto pb-1 scrollbar-hide">
                    {movies.map((m, i) => (
                        <button key={m.id} onClick={() => setActiveTrailer(i)}
                            className={`relative shrink-0 w-36 h-20 rounded-[var(--radius-card)] overflow-hidden border-2 transition-all ${activeTrailer === i ? "border-[var(--accent)]" : "border-transparent hover:border-white/30"}`}>
            ...
            {current && (
                <p className="text-xs text-white/45 mt-3">{current.name}</p>
            )}
        </div>
    );
}
