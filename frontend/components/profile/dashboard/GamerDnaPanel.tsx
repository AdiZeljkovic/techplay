    ...
    return (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            <Panel title="Favorite Genres">
                {dna.genres?.length > 0 ? <DistributionBars items={dna.genres} /> : <p className="text-[11px] text-white/25">No data</p>}
            </Panel>
            <Panel title="Favorite Platform">
                {dna.platforms?.length > 0 ? <DistributionBars items={dna.platforms} barClassName="bg-gradient-to-r from-violet-500 to-fuchsia-500" /> : <p className="text-[11px] text-white/25">No data</p>}
            </Panel>
            <Panel title="Playstyle">
                {dna.playstyle?.length > 0 ? (
                    <div className="flex flex-wrap gap-2">
                        {dna.playstyle.map((p) => (
                            <span key={p} className="text-[11px] font-semibold text-[var(--accent)] bg-[var(--accent)]/10 border border-[var(--accent)]/30 px-2.5 py-1 rounded-full">{p}</span>
                        ))}
                    </div>
