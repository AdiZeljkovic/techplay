...
export default function CustomLists({ lists }: { lists: GameListPreview[] }) {
    return (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {lists.map((l) => (
                <Link key={l.id} href="?tab=lists" className="group relative rounded-[var(--radius-card)] overflow-hidden border border-white/[0.06] bg-[var(--surface-1)] aspect-[16/10] hover:border-[var(--accent)]/40 transition-colors">
                    {l.covers?.[0] ? (
                    ...
                    <div className="absolute bottom-0 left-0 right-0 p-3 flex items-center gap-2.5">
                        <span className="w-7 h-7 rounded-[var(--radius-inner)] bg-black/40 border border-white/[0.08] flex items-center justify-center shrink-0">
