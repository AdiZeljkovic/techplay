...
export default function ProfileStats({ stats, isStaff }: ProfileStatsProps) {
    const statItems = [
        ...
        { icon: MessageSquare, label: "Threads", value: stats.threads_count, color: "#FC4100", bg: "rgba(252,65,0,0.08)" },
        { icon: MessageSquare, label: "Posts", value: stats.posts_count, color: "#FF7A3D", bg: "rgba(255,122,61,0.08)" },
        { icon: Trophy, label: "Achievements", value: stats.achievements_count, color: "#facc15", bg: "rgba(250,204,21,0.08)" },
        { icon: Star, label: "Reputation", value: stats.reputation, color: "#d43600", bg: "rgba(212,54,0,0.08)" },
    ];

    if (isStaff && stats.reviews_count) {
        statItems.push({ icon: Pen, label: "Articles", value: stats.reviews_count, color: "#FC4100", bg: "rgba(252,65,0,0.08)" });
    }
    ...
    return (
        <div className="relative bg-gradient-to-b from-[var(--bg-secondary)] to-[var(--bg-primary)]">
            ...
            <div className="relative max-w-[1320px] mx-auto px-4 xl:px-0 py-5 md:py-6">
