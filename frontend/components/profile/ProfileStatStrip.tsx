...
export default function ProfileStatStrip({ stats, nextXp }: Props) {
    const items = [
        ...
        { icon: Flame, label: "XP", value: (stats.xp ?? 0).toLocaleString(), sub: nextXp ? `/ ${nextXp.toLocaleString()}` : undefined, color: "#fb923c" },
        { icon: Library, label: "Games in Collection", value: stats.games_count ?? 0, color: "#a78bfa" },
        { icon: Gamepad2, label: "Playing Now", value: stats.playing_count ?? 0, color: "#34d399" },
                ...
                {items.map((s) => (
                    ...
                    >
                        ...
                        <span className="text-lg font-black text-white tabular-nums leading-none">
                            {s.value}
                            {s.sub && <span className="text-[10px] font-bold text-white/30 ml-0.5">{s.sub}</span>}
                        </span>
                        <span className="text-[8px] uppercase tracking-[0.12em] text-white/35 font-bold text-center leading-tight min-h-[1.6em] flex items-center">{s.label}</span>
                    </div>
                ))}
            </div>
        </div>
