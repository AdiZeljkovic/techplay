...
import { MessageSquare, MessageCircle, Trophy, Gamepad2, ListChecks, Gift, Sparkles, Activity as ActivityIcon } from "lucide-react";
...
const TYPE_META: Record<string, { icon: any; color: string }> = {
    ...
    comment: { icon: MessageCircle, color: "#FF7A3D" },
    achievement: { icon: Trophy, color: "#facc15" },
    game: { icon: Gamepad2, color: "#34d399" },
    list: { icon: ListChecks, color: "#60a5fa" },
...
interface Props {
    username: string;
    /** Compact = overview preview (no filters/pagination, capped). */
    compact?: boolean;
}
...
export default function ActivityFeed({ username, compact }: Props) {
    ...
    const perPage = compact ? 6 : 15;
    const query = `category=${filter === "all" ? "" : filter}&page=${page}&per_page=${perPage}`;
        ...
        <div>
            ...
            {isLoading ? (
                <div className="space-y-2">
                    {Array.from({ length: compact ? 4 : 8 }).map((_, i) => <div key={i} className="h-12 bg-white/[0.04] rounded-lg animate-pulse" />)}
                </div>
            ) : items.length === 0 ? (
