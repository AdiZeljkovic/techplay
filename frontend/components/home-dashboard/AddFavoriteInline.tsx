        ...
        <div className="flex-1 min-h-[104px] flex flex-col rounded-xl border border-[var(--line-strong)] bg-[var(--fill-1)] p-3">
            <div className="flex items-center gap-2 h-9 px-3 rounded-lg bg-black/30 border border-[var(--line-strong)] focus-within:border-[var(--accent)]/50 transition-colors">
                ...
                <input
                    ...
                    className="flex-1 min-w-0 bg-transparent text-[13px] text-white placeholder:text-[var(--ink-faint)] outline-none"
                />
                {searching && <Loader2 className="w-3.5 h-3.5 text-[var(--ink-low)] animate-spin shrink-0" />}
                <button onClick={() => { setOpen(false); setQuery(""); onDismiss?.(); }} aria-label="Cancel" className="shrink-0 text-[var(--ink-faint)] hover:text-white transition-colors">
                    <X className="w-4 h-4" />
                </button>
            </div>
            ...
            <div className="mt-2 flex-1 overflow-y-auto max-h-[176px] scrollbar-none">
                {debounced.length < 2 && (mine?.length ?? 0) > 0 && (
                    <p className="px-1 py-1 text-[10px] font-bold uppercase tracking-wider text-[var(--ink-faint)]">From your library</p>
                )}
                ...
                {shown?.map((g) => (
                    ...
                    >
                        ...
                        {busySlug === g.slug ? (
                            ...
                            <Star className="w-3.5 h-3.5 text-[var(--ink-faint)] group-hover:text-[var(--accent)] group-hover:fill-[var(--accent)] transition-colors shrink-0" />
                        )}
                    </button>
                ))}

                {debounced.length >= 2 && !searching && (results?.length ?? 0) === 0 && (
                    <p className="py-4 text-center text-[12px] text-[var(--ink-faint)]">No games found for “{debounced}”</p>
                )}
                {debounced.length < 2 && (mine?.length ?? 0) === 0 && (
                    <p className="py-4 text-center text-[12px] text-[var(--ink-faint)]">Type at least 2 letters to search</p>
                )}
            </div>
        </div>
    );
