...
export default function Leaderboard({ slug }: LeaderboardProps) {
    ...
    const getRankIcon = (rank: number) => {
        switch (rank) {
            ...
            case 2:
                return <Medal className="w-5 h-5 text-white/45" />;
            case 3:
    ...
    const getRankColor = (rank: number) => {
        switch (rank) {
            ...
            case 2:
                return "from-white/45/20 to-white/35/20 border-white/45/50";
            case 3:
                return "from-orange-400/20 to-red-500/20 border-orange-400/50";
            default:
                return "from-[var(--surface-2)] to-[var(--surface-2)] border-[var(--line)]";
        }
    };

    if (loading) {
        return (
            <div className="glass-card rounded-[var(--radius-panel)] p-6">
        ...
        return (
            <div className="glass-card rounded-[var(--radius-panel)] p-6">
                <h3 className="text-lg font-bold mb-2 flex items-center gap-2">
                    <div className="w-8 h-8 rounded-[var(--radius-card)] bg-gradient-to-br from-[var(--accent)]/20 to-purple-500/20 flex items-center justify-center">
    ...
    return (
        <div className="glass-card rounded-[var(--radius-panel)] p-6 relative overflow-hidden group">
            ...
            <div className="relative">
                ...
                <div className="space-y-3">
                    {entries.map((entry, index) => (
                        ...
                        >
                            <div className={`absolute inset-0 rounded-[var(--radius-card)] blur-lg transition-opacity duration-300 ${
                                entry.rank <= 3 ? 'opacity-30 group-hover/item:opacity-50' : 'opacity-0 group-hover/item:opacity-20'
                            } ${
                                entry.rank === 1 ? 'bg-yellow-500/30' :
                                entry.rank === 2 ? 'bg-white/45/30' :
                                entry.rank === 3 ? 'bg-orange-400/30' :
                                'bg-[var(--accent)]/20'
                            }`} />
                            <div className={`relative/50 border rounded-[var(--radius-card)] p-3 transition-all duration-300 hover:scale-[1.02] ${
                                entry.rank === 1 ? 'border-yellow-500/50' :
                                entry.rank === 2 ? 'border-white/45/50' :
                                ...
                                <div className="flex items-center gap-3">
                                    ...
                                    <div className="flex-shrink-0 w-8 flex items-center justify-center">
                                        {getRankIcon(entry.rank) || (
                                            <span className="font-bold text-white/55 text-sm">
                                                #{entry.rank}
                                            </span>
                                        )}
                                    </div>
                                    ...
                                    <div className={`w-10 h-10 rounded-full flex items-center justify-center text-white font-bold flex-shrink-0 ${
                                        ...
                                        entry.rank === 2 ? 'bg-gradient-to-br from-white/45 to-white/35' :
