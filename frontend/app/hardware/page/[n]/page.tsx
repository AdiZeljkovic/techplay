 ...
 *
 * /hardware renders thirteen articles and its pager was a pair of buttons, so 587
 ...
 * saying it mattered.
 *
 * Page one stays at /hardware, static and cached; there is no /hardware/page/1, so each
...
function parsePage(raw: string): number | null {
    ...
    // Page one is /hardware. Serving it here too would be the same list at two
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
    ...
    const base = await generatePageMetadata(`/hardware/page/${page}`, {
        title: `Hardware — page ${page}`,
        description: `GPU, CPU and component reviews with benchmark data. Page ${page} of the archive.`,
    });

    return { ...base, robots: ROBOTS_INDEX };
}

export default async function HardwarePagedPage({ params }: Props) {
    const { n } = await params;
    const page = parsePage(n);

    if (page === null) notFound();

    const data = await fetchSectionPage("tech", page);

    // Past the end of the archive is not a page. Without this, /hardware/page/900
