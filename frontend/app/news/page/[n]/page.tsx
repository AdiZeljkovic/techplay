 ...
 *
 * /news renders thirteen articles and its pager was a pair of buttons, so 587
 ...
 * saying it mattered.
 *
 * Page one stays at /news, static and cached; there is no /news/page/1, so each
...
function parsePage(raw: string): number | null {
    ...
    // Page one is /news. Serving it here too would be the same list at two
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
    ...
    const base = await generatePageMetadata(`/news/page/${page}`, {
        title: `Gaming News — page ${page}`,
        description: `Game announcements, release dates, patches and industry news. Page ${page} of the archive.`,
    });
...
export default async function NewsPagedPage({ params }: Props) {
    ...
    const data = await fetchSectionPage("news", page);

    // Past the end of the archive is not a page. Without this, /news/page/900
