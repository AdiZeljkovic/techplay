 ...
 *
 * /reviews renders thirteen articles and its pager was a pair of buttons, so 587
 ...
 * saying it mattered.
 *
 * Page one stays at /reviews, static and cached; there is no /reviews/page/1, so each
...
function parsePage(raw: string): number | null {
    ...
    // Page one is /reviews. Serving it here too would be the same list at two
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
    ...
    const base = await generatePageMetadata(`/reviews/page/${page}`, {
        title: `Game Reviews — page ${page}`,
        description: `Scored reviews with measured numbers behind them. Page ${page} of the archive.`,
    });

    return { ...base, robots: ROBOTS_INDEX };
}

export default async function ReviewsPagedPage({ params }: Props) {
    const { n } = await params;
    const page = parsePage(n);

    if (page === null) notFound();

    const data = await fetchSectionPage("reviews", page);

    // Past the end of the archive is not a page. Without this, /reviews/page/900
