...
async function handleCommand(interaction, client) {
    const api = ApiService_1.ApiService.getInstance();
...
async function handleHelp(interaction, buffy) {
    const embed = buffy.createHelpEmbed();
    await interaction.reply({ embeds: [embed] });
}
async function handleTip(interaction, buffy) {
    const embed = buffy.createTipEmbed();
    await interaction.reply({ embeds: [embed] });
}
async function handleProfile(interaction, api, buffy) {
...
async function handleDaily(interaction, api, buffy) {
    await interaction.deferReply();
    ...
    const embed = buffy.createDailyBonusEmbed(interaction.user.username, result.xp_awarded || 50, result.streak || 1, result.total_xp || 0);
    await interaction.editReply({ embeds: [embed] });
}
async function handleStats(interaction, api, buffy, client) {
    await interaction.deferReply();
...
async function handleLeaderboard(interaction, api, buffy) {
    await interaction.deferReply();
    const leaderboard = await api.getLeaderboard();
    ...
    const embed = buffy.createLeaderboardEmbed(leaderboard.map((u, index) => ({
        position: index + 1,
...
async function handleTrivia(interaction) {
    const triviaService = TriviaService_1.TriviaService.getInstance();
    await triviaService.startTrivia(interaction);
}
async function handleSearch(interaction, api, buffy) {
    await interaction.deferReply();
    ...
    const content = results.slice(0, 5).map(r => `🔹 **[${r.title}](https://techplay.gg${r.url || `/news/${r.slug}`})**\n${r.excerpt || ''}`).join('\n\n');
    await interaction.editReply({
        content: `🔍 **Search results for "${query}"**\n\n${content}`
    });
}
async function handleGame(interaction, api, buffy) {
    await interaction.deferReply();
...
async function handleTechplay(interaction, api) {
    await interaction.deferReply();
...
async function handleLatest(interaction, api) {
    await interaction.deferReply();
...
async function handleGiveaways(interaction, api) {
    await interaction.deferReply();
...
async function handleForum(interaction, api) {
    await interaction.deferReply();
...
async function handleSubscribe(interaction, buffy) {
...
async function handleGift(interaction, api, buffy) {
    await interaction.deferReply();
...
async function handleChallenge(interaction, client) {
    ...
    const embed = await challengeService.createChallenge(interaction.user, targetUser, interaction.channel);
    if (embed) {
        await interaction.reply({ embeds: [embed] });
    }
}
async function handleAdmin(interaction, api, buffy, client) {
    ...
    if (sub === 'announce') {
        ...
        const announcementChannel = interaction.guild?.channels.cache.find(c => c.name === 'announcements' || c.name === 'general');
