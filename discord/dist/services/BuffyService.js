...
class BuffyService {
    ...
    createWelcomeEmbed(username, memberCount) {
        return new discord_js_1.EmbedBuilder()
            .setTitle(`${this.getWelcomeMessage()}`)
            .setDescription(`Welcome to **TechPlay**, **${username}**! 🎉\n\n` +
    ...
    createLevelUpEmbed(username, newRank, totalXp) {
        return new discord_js_1.EmbedBuilder()
            .setTitle(`🎉 RANK UP!`)
            .setDescription(`${this.getLevelUpMessage()}\n\n` +
    ...
    createAchievementEmbed(username, achievementName, description) {
        return new discord_js_1.EmbedBuilder()
            .setTitle(`🏆 Achievement Unlocked!`)
            .setDescription(`${this.getAchievementMessage()}\n\n` +
            `**${username}** earned:\n` +
            `**${achievementName}**\n` +
            `*${description}*`)
    ...
    createAlreadyClaimedEmbed(hoursLeft) {
        return new discord_js_1.EmbedBuilder()
            .setTitle(`⏰ Already Claimed!`)
            .setDescription(`${this.getAlreadyClaimedMessage()}\n\n` +
    ...
    createTipEmbed() {
        ...
        return new discord_js_1.EmbedBuilder()
            .setTitle(`${category}`)
            .setDescription(`*Professor Buffy adjusts his spectacles and speaks:*\n\n` +
            `"${tip}"\n\n` +
    ...
    createProfileEmbed(data) {
        ...
        const embed = new discord_js_1.EmbedBuilder()
            ...
            .addFields({ name: '🏅 Rank', value: data.rank, inline: true }, { name: '⭐ XP', value: data.xp.toLocaleString(), inline: true }, { name: '🏆 Position', value: data.position ? `#${data.position}` : 'Unranked', inline: true }, { name: '📊 Progress to Next Rank', value: `${progressBar} ${progress}%`, inline: false })
            .setColor(BuffyService.COLORS.PRIMARY)
    ...
    // SERVER STATS EMBED
    // ═══════════════════════════════════════════════════════════════════
    createServerStatsEmbed(stats) {
        return new discord_js_1.EmbedBuilder()
            .setTitle(`📊 TechPlay Server Statistics`)
            .setDescription(`*Professor Buffy presents the community metrics!*`)
            .addFields({ name: '👥 Total Members', value: stats.totalMembers.toLocaleString(), inline: true }, { name: '🟢 Online Now', value: stats.onlineMembers.toLocaleString(), inline: true }, { name: '🔗 Linked Accounts', value: stats.linkedAccounts.toLocaleString(), inline: true }, { name: '⭐ Total XP Awarded', value: stats.totalXpAwarded.toLocaleString(), inline: true }, { name: '💬 Messages This Week', value: stats.messagesThisWeek.toLocaleString(), inline: true })
            .setColor(BuffyService.COLORS.PRIMARY)
            .setThumbnail(BuffyService.AVATAR_URL)
            .setFooter({ text: BuffyService.FOOTER_TEXT })
            .setTimestamp();
    ...
    createHelpEmbed() {
        return new discord_js_1.EmbedBuilder()
            ...
            .setDescription(`*Greetings, young gamer! Here are the commands at your disposal:*\n\n` +
            `**📊 Profile & Stats**\n` +
    ...
    createErrorEmbed(message) {
        return new discord_js_1.EmbedBuilder()
            .setTitle(`❌ Oops!`)
            .setDescription(`*Professor Buffy scratches his head...*\n\n` +
            `${message}\n\n` +
    ...
    createNotLinkedEmbed() {
        return new discord_js_1.EmbedBuilder()
            .setTitle(`🔗 Account Not Linked`)
            .setDescription(`*Professor Buffy checks his records...*\n\n` +
    ...
    // ═══════════════════════════════════════════════════════════════════
    createNewsNotificationEmbed(title, excerpt, url) {
        return new discord_js_1.EmbedBuilder()
            .setTitle(`📰 New Article on TechPlay!`)
            .setDescription(`*Hoot! Professor Buffy has news for you!*\n\n` +
            `**${title}**\n\n` +
            `${excerpt}\n\n` +
    ...
    createGiveawayNotificationEmbed(title, endDate, url) {
        return new discord_js_1.EmbedBuilder()
            .setTitle(`🎁 New Giveaway Started!`)
            .setDescription(`*Hoot hoot! Free stuff alert!*\n\n` +
            `**${title}**\n\n` +
            `⏰ Ends: ${endDate}\n\n` +
            `[🎯 Enter Now!](${url})\n\n` +
    ...
    createSubscriptionEmbed(subscribed, type) {
        ...
        return new discord_js_1.EmbedBuilder()
            ...
            .setDescription(`*Professor Buffy updates his notification list...*\n\n` +
    ...
    // GIFT EMBEDS
    // ═══════════════════════════════════════════════════════════════════
    createGiftEmbed(senderName, receiverName, amount) {
        return new discord_js_1.EmbedBuilder()
            .setTitle(`🎁 XP Gift!`)
            .setDescription(`*Professor Buffy witnesses a generous act!*\n\n` +
    ...
    createGiftErrorEmbed(message) {
        return new discord_js_1.EmbedBuilder()
            .setTitle(`🎁 Gift Failed`)
            .setDescription(`*Professor Buffy shakes his head...*\n\n` +
            `${message}`)
            .setColor(BuffyService.COLORS.ERROR)
            .setThumbnail(BuffyService.AVATAR_URL)
            .setFooter({ text: BuffyService.FOOTER_TEXT })
    ...
    // ═══════════════════════════════════════════════════════════════════
    createChallengeEmbed(challengerName, opponentName) {
        return new discord_js_1.EmbedBuilder()
            .setTitle(`⚔️ Trivia Duel Challenge!`)
            .setDescription(`*Professor Buffy announces a battle of wits!*\n\n` +
    ...
    createDuelQuestionEmbed(question, category, questionNum, totalQuestions) {
        return new discord_js_1.EmbedBuilder()
    ...
    createDuelResultEmbed(winnerName, player1, player1Score, player2, player2Score, xpReward) {
        ...
        return new discord_js_1.EmbedBuilder()
            ...
            .setDescription(`*Professor Buffy tallies the scores...*\n\n` +
    ...
    // ADMIN EMBEDS
    // ═══════════════════════════════════════════════════════════════════
    createAdminXpEmbed(action, adminName, targetName, amount, newTotal) {
        const emoji = action === 'give' ? '➕' : '➖';
        const verb = action === 'give' ? 'granted' : 'removed';
        return new discord_js_1.EmbedBuilder()
            .setTitle(`${emoji} Admin XP ${action === 'give' ? 'Grant' : 'Removal'}`)
            .setDescription(`**${adminName}** ${verb} **${amount} XP** ${action === 'give' ? 'to' : 'from'} **${targetName}**\n\n` +
    ...
    createAnnouncementEmbed(message) {
        return new discord_js_1.EmbedBuilder()
            .setTitle(`📢 Community Announcement`)
            .setDescription(`*Professor Buffy clears his throat...*\n\n` +
            `${message}`)
            .setColor(BuffyService.COLORS.PRIMARY)
            .setThumbnail(BuffyService.AVATAR_URL)
            .setFooter({ text: BuffyService.FOOTER_TEXT })
    ...
    createEventEmbed(eventName, duration, active) {
        if (active) {
            return new discord_js_1.EmbedBuilder()
                .setTitle(`🎉 Event Started: ${eventName}!`)
                .setDescription(`*Professor Buffy waves his wand...*\n\n` +
            ...
            return new discord_js_1.EmbedBuilder()
                ...
                .setDescription(`*Professor Buffy waves goodbye...*\n\n` +
                `**${eventName}** has ended!\n\n` +
...
exports.BuffyService = BuffyService;
...
BuffyService.AVATAR_URL = 'https://techplay.gg/images/buffy-avatar.png';
BuffyService.FOOTER_TEXT = '🦉 Professor Buffy | TechPlay Community';
